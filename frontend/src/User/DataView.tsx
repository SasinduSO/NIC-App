...
import CustomNavbar from '../shared/ustomNavbar'; // Ensure correct import
...
const DataView: React.FC = () => {
  ...
  const currentItems = nics.slice(indexOfFirstItem, indexOfLastItem);
      ...
      <div className="mt-16 px-4 py-8 bg-[#071952] min-h-screen">
          ...
          <div className="bg-[#51829B] rounded-lg shadow-md p-3 flex items-center justify-between">
            ...
            <div className="flex items-center ml-4 text-black">
              <FormControlLabel
                control={
                  <Switch
                    checked={state.Valid} // Set the toggle state according to the current page view
                    onChange={handleToggle}
                    name="Valid"
                  />
                }
                label="Toggle to View Invalidated NIC data"
              />
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 p-2 rounded-lg">
          <ReactPaginate
            ...
            nextLabel={"Next"}
            breakLabel={"..."}
            pageCount={Math.ceil(nics.length / itemsPerPage)}
